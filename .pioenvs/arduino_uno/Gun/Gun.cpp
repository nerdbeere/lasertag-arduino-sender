...
bool Gun::isReloading() {
    return _state == STATE_RELOADING;
}

void Gun::checkReload() {
    if(_state == STATE_RELOADING && millis() > _reloadUntil) {
        _state = STATE_READY_TO_FIRE;
        _currentAmmoInMag = AMMO_IN_MAG;
        playSound(2);
    }
...
bool Gun::shoot() {
    if(_state != STATE_READY_TO_FIRE || checkBulletInChamber() != true) {
        return false;
    }
...
void Gun::checkHitIndicator() {
    if(millis() > _time && _state == STATE_READY_TO_FIRE) {
        hitIndicatorOff();
    }
}
